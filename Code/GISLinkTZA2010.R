...
# NB: Note that the GADM for TZA was updated/changed end of 2015. We need to use the old saved version for consistency!
country.map <- Get.country.shapefile.f(iso3c, 2)

# PREPARE LSMS SPATIAL DATAPOINTS
# Get y2_hhid-GIS link
HH.geo <- read_dta(file.path(dataPath, "TZNPS2GEODTA/HH.Geovariables_Y2.dta")) %>%
            rename(lat = lat_modified, lon = lon_modified, eaid = ea_id)
plot.geo <- read_dta(file.path(dataPath,"TZNPS2GEODTA/Plot.Geovariables_Y2.dta")) %>%
            rename(eaid = ea_id)
...
# The TZA BID document points out that for some households the information pertains
# to 2010 long rainy season and for others to 2011 season depending on the time of the interview.
# It is not clear which households answer for 2010 and which for 2011. THe only information
# there is is the data of the interview. THe questionnaire itself refers to the 2010 season.
# So it is not clear if households answer on the 2010 season or the latest completed season.
# An analysis of the time of interview data (below) idnicates that 157 households were 
# interviewed after September 2011 after which the 2011 harvest is probably completed.
# We decide to take the 2010 year rainfall for the SPEI index as the majority of the households
# answer questions on this season.
...
# Data is missing for Zanzibar and for a 5 locations that are located close to victoria lake
...
geo.total.plot <- geo.total.plot %>%
                transmute(y2_hhid, lon, lat, plotnum, SPEI, RootDepth, region_name=NAME_1, district_name=NAME_2,
                          ...
                          elevation=soil01, slope=soil02, twi=soil03, nut = soil05, nrc = soil06, # Note that soil, elevation and twi are based on hh, not plot location as in 2012
